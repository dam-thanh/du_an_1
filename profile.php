...
if (isset($_POST['insert'])) {
  ...
  if (!empty($name) && !empty($phone) && !empty($content)) {
    try {
      ...
      // Sử dụng truy vấn sử dụng PDO và tham số đặt tên
      $query = "INSERT INTO feedback (uid, fdescription, status) VALUES (:uid, :content, '0')";
      ...
      // Xử lý ngoại lệ nếu có lỗi kết nối hoặc thực thi truy vấn
      $error = "<p class='alert alert-warning'>An error occurred: " . $e->getMessage() . "</p>";
    }
  } else {
